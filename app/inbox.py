...
def show():
    ...
    db = get_db()
    messages = db.execute(
        'SELECT m.id, subject, body, created, from_id, username'
        ' FROM message m JOIN user u ON m.from_id = u.id'
        ' WHERE to_id = ?'
        ' ORDER BY created DESC', (to_id,)
    ).fetchall()
...
def send():
    if request.method == 'POST':        
        ...
        if not to_username:
            flash('To field is required','error')
            return render_template('inbox/send.html')
        
        if not subject:
            flash('Subject field is required','error')
            return render_template('inbox/send.html')
        
        if not body:
            flash('Body field is required','error')
            return render_template('inbox/send.html')  
